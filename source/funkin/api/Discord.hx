...
class DiscordClient
{
		...
		while (true) {
			...
			if (msg==false) {
				...
				if (isActive != true) {
					...
					DiscordRpc.Shutdown();
					var eventHandler:DiscordEventHandlers = #if (hxdiscord_rpc > "1.2.4") new DiscordEventHandlers(); #else DiscordEventHandlers.create(); #end
					...
					DiscordRpc.Initialize(curId, cpp.RawPointer.addressOf(eventHandler), #if (hxdiscord_rpc > "1.2.4") false #else 1 #end, null);
	...
	{
		var presence = #if (hxdiscord_rpc > "1.2.4") new DiscordRichPresence(); #else DiscordRichPresence.create(); #end
	...
	public static function changePresence(details:String, ?state:String, ?largeImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float)
	{
		...
		lastPresence = #if (hxdiscord_rpc > "1.2.4") new DiscordRichPresence(); #else DiscordRichPresence.create(); #end
